const tl_rtc_app_module_channel_list = {
        ...
        filterChannels(){
            ...
            if(this.searchTag.system){
                ...
                if(tagName === '全部'){
                    ...
                    return resultList.filter(item => item.channelTop)
                }else if(tagName === '未读'){
                    return resultList.filter(item => item.channelChatUnReadCount > 0)
                }else if(tagName === '群聊'){
                    return resultList.filter(item => item.channelType === 'group')
                }else if(tagName === '单聊'){
                    return resultList.filter(item => item.channelType === 'friend')
                }else if(tagName === '拉黑'){
                    return resultList.filter(item => item.channelBlack)
                }
            }
    ...
    methods: {
        ...
        channelChatRollbackSocketHandler: function(data){
            const { channel, messageId, rollbackMessage } = data
            if(!channel || !messageId || !rollbackMessage){
                data.callback && data.callback()
                return
            }
            console.log('channelChatRollbackSocketHandler', data)

            // 移除频道消息
            this.channelsMessageMap[channel] = this.channelsMessageMap[channel].filter(item => item.id !== messageId)

            this.addChannelMessageHandler({
                channelId: channel, messageInfo: rollbackMessage
            })
        },
        /**
         * 添加频道消息 - 公共处理逻辑
        ...
        updateChannelUnReadMessage: async function(channelId, event){
            ...
            if(!readRes.success){
                this.popErrorMsg(readRes.message)
                return
            }
        ...
        */
         ...
         * @param {*} callback 
         */
        rollbackChannelMessageHandler: async function({
            messageId, channelId, rollbackMessage, callback
        }){
            let messageList = this.channelsMessageMap[channelId]

            if(messageList && messageList.length > 0){  
                messageList = messageList.filter(item => item.id !== messageId)
                this.channelsMessageMap[channelId] = messageList
            }

            // 新增一条撤回消息
            messageList.push(rollbackMessage)
            this.channelsMessageMap[channelId] = messageList
    ...
    created: async function(){
        ...
        // 监听收到socket聊天消息撤回
        window.subModule.$on('sub-module-channel-list-rollback-channel-message-socket', this.channelChatRollbackSocketHandler)

        // 移动端返回到频道列表
        window.subModule.$on('sub-module-channel-list-go-back', this.channelListGoBack)

        // 撤回消息
        window.subModule.$on('sub-module-channel-list-rollback-channel-message', this.rollbackChannelMessageHandler)
    },
    template: `
        <div class="tl-rtc-app-left-panel-channel">
            <div v-if="requestOk && filterChannels.length === 0">
                            ...
                            <div class="swiper-wrapper">
                                ...
                                >
                                    ...
                                    <div class="tl-rtc-app-left-panel-channel-item-content">
                                        ...
                                        <div class="tl-rtc-app-left-panel-channel-item-content-message"
                                            v-else-if="['audio', 'video', 'screen', 'live', 'voice'].includes(item.extra.type)"
                                        >
                                            <svg class="icon" aria-hidden="true">
