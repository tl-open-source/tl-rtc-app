...
const { model, fields } = require('../tables/tl_cut_paste_detail')
const Table = model(dbClient, DataTypes).TlCutPasteDetail
const TableName = fields.Name
const TableFields = Object.keys(fields.Def).map(key => fields.Def[key])
const { tlConsoleError, tlConsole, checkRawFieldType } = require("../utils/utils");
 ...
 * @param {*} group
 */
const getGroupCount = async function(params, group){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数有为空项")
        return null
    }
