...
const { model, fields } = require('../tables/tl_role')
const Table = model(dbClient, DataTypes).TlRole
 ...
 * @param {*} pageSize
 */
const getListForPage = async function(params, fields = TableFields, order = [['id', 'ASC']], page = 1, pageSize){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数有为空项")
        return null
    }
