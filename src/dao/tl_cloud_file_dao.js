...
const { model, fields } = require('../tables/tl_cloud_file')
const Table = model(dbClient, DataTypes).TlCloudFile
const TableName = fields.Name
const TableDef = fields.Def
 ...
 * @returns 
 */
const getInfoDeleted = async function(params, fields = TableFields){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数为空")
        return null
    }
 ...
 * @param {*} order
 */
const getListDeleted = async function(params, fields = TableFields, order = [['id', 'ASC']]){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数有为空项")
        return null
    }
 ...
 * @param {*} pageSize
 */
const getListDeletedForPage = async function(params, fields = TableFields, order = [['id', 'ASC']], page = 1, pageSize = 10){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数有为空项")
        return null
    }
 ...
 * @param {*} params 
 */
const restoreInfo = async function(params){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数为空")
        return null
    }

    const info = await Table.restore({
        where: params
    })
 ...
 * @param {*} params
 */
const getCountDeleted = async function(params){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数有为空项")
        return null
    }
 ...
 * @returns 
 */
const deleteInfoForce = async function(params){
    if(Object.keys(params).length === 0){
        tlConsoleError(TableName, "请求dao参数为空")
        return null
    }
...
module.exports = {
    addInfo, addList,
    getInfo, getList, getListDeleted, getListForPage, getListDeletedForPage, getInfoDeleted,
    updateInfo,
    deleteInfo, restoreInfo, deleteInfoForce,
    getCount, getCountDeleted
}