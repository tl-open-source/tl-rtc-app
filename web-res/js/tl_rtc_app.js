...
let load_tl_rtc_app_js = async function(){
    ...
    if(isLogin){
        ...
        let { data : userConfigRes } = await Vue.prototype.tlRequest({
            url: '/api/web/user-config/get-user-config',
            method: 'get',
            useCache: false,
        })
        normalSetting = userConfigRes.data.normal
        accountSetting = userConfigRes.data.account
        messageSetting = userConfigRes.data.message
        skinSetting = userConfigRes.data.skin
        otherSetting = userConfigRes.data.other
        // 默认首页
        homePage = normalSetting.homePage || 'channel'
        // 是否使用自定义wss地址
    ...
    window.tl_rtc_app = new Vue({
        ...
        data: function(){
            ...
            if (io && isLogin) {
                ...
                this.emitSubModuleEvent({
                    event: "component-rtc-init-socket",
                    data: {
                        socket, rtcConfig
                    }
                })
                this.emitSubModuleEvent({
                    event: "component-video-init-socket",
                    data: {
                        socket
                    }
                })
            ...
            return {
                ...
                leftModule : isLogin ? homePage : 'login', // 左侧默认模块
        ...
        methods: {
            ...
            userSettingChange: function({
                normalSetting, accountSetting, messageSetting, 
                skinSetting, otherSetting
            }){
                if(normalSetting){
                    this.user.normalSetting = normalSetting
                }
                if(accountSetting){
                ...
                if(skinSetting){
                    this.user.skinSetting = skinSetting
                }
                if(otherSetting){
