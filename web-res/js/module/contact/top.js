const tl_rtc_app_module_contact_top = {
    ...
    methods: {
        ...
        searchGroup: async function({
            ...
            layer.prompt({
                ...
                const { data: groupRes } = await that.tlRequest({
                ...
                if(!groupRes.success){
                    that.popErrorMsg(groupRes.msg)
                    return false;
                }

                that.popSuccessMsg(groupRes.msg)
    ...
    created(){
        ...
        // 监听搜索群聊事件
        window.subModule.$on('sub-module-contact-top-search-group', this.searchGroup)
    },
    template: `
