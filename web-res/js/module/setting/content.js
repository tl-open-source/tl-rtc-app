const tl_rtc_app_module_setting_content = {
    ...
    methods: {
         ...
         * 更新用户通用设置
         */
        updateUserConfigNormal: async function(updateNormalSetting){
            let params = {
                normal: updateNormalSetting
            }
            ...
            const { data: userConfigRes } = await this.tlRequest({
                url: '/api/web/user-config/update-user-config-normal',
                method: 'post',
                useCache: false,
                data: params,
            ...
            this.emitSubModuleEvent({
                ...
                data: {
                    normalSetting: updateNormalSetting
                }
            })
         ...
         * 更新用户消息设置
         */
        updateUserConfigMessage: async function(updateMessageSetting){
            let params = {
                message: updateMessageSetting
            }

            if(updateMessageSetting.webNotify){
                if (window.Notification) {
                    ...
                    that.popWarningMsg('浏览器不支持消息通知')
                    updateMessageSetting.webNotify = false
                }
            }
            ...
            const { data: userConfigRes } = await this.tlRequest({
                url: '/api/web/user-config/update-user-config-message',
                method: 'post',
                useCache: false,
                data: params,
            })
            ...
            this.emitSubModuleEvent({
                event: 'sub-module-core-user-setting-change',
                data: {
                    messageSetting: updateMessageSetting
                }
            })
         ...
         * 更新用户皮肤设置
         */
        updateUserConfigSkin: async function(udpateSkinSetting){
            let params = {
                skin: udpateSkinSetting
            }
            if(!window.tl_rtc_app_comm.checkRequestParams(params)){
                this.popErrorMsg("请求参数非法")
                return
            ...
            const { data: userConfigRes } = await this.tlRequest({
                url: '/api/web/user-config/update-user-config-skin',
                method: 'post',
                useCache: false,
                data: params,
    ...
    components : {
        'tl_rtc_app_module_setting_content_normal': window.tl_rtc_app_module_setting_content_normal,
        ...
        'tl_rtc_app_module_setting_content_skin': window.tl_rtc_app_module_setting_content_skin,
    ...
    template: `
        <div class="tl-rtc-app-right-setting-content">
            ...
            <tl_rtc_app_module_setting_content_normal
                v-if="settingType == 'normal'"
                :socket="propsSocket"
                :is-mobile="propsIsMobile"
                :normal-setting="propsUser.normalSetting"
                @update-user-config-normal="updateUserConfigNormal"
            ...
            <tl_rtc_app_module_setting_content_skin
                v-show="settingType == 'skin'"
                :socket="propsSocket"
                :is-mobile="propsIsMobile"
                :skin-setting="propsUser.skinSetting"
                @update-user-config-skin="updateUserConfigSkin"
                :company="propsCompany"
                :user="propsUser"
                :left-module="propsLeftModule"
                :right-module="propsRightModule"
